...
big_integer &big_integer::trivial_multiplication::multiply(
    big_integer &first,
    big_integer const &second) const
{
    if (second.is_equal_to_zero()) {
        return first = second;
    }
    
    if (first.is_equal_to_zero()) {
        return first;
    }
    
    if (second.is_equal_to_one()) {
        return first;
    }
    
    if (first.is_equal_to_one()) {
        return first = second;
    }

    if (first.sign() == -1) {
        return multiply(first.change_sign(), second).change_sign();
    }
    
    if (second.sign() == -1) {
        return multiply(first, -second).change_sign();
    }

    if (second._oldest_digit == -1 && second._other_digits == nullptr) {
        return (first.change_sign());
    }

    constexpr unsigned half_of_uint = sizeof(unsigned) << 2;
    constexpr unsigned half_mask = (1 << half_of_uint) - 1;

    std::vector<unsigned> results(first.get_digits_count() + second.get_digits_count() + 2);
    
    auto mult_and_add_to_vec = [&](
        unsigned first,
        unsigned second,
        unsigned index) 
    {
        unsigned a_low = first & half_mask;
        unsigned a_high = first >> half_of_uint;
        unsigned b_low = second & half_mask;
        unsigned b_high = second >> half_of_uint;

        unsigned buf = 
            a_low * b_low + 
            (results[index] & half_mask);

        unsigned new_low = buf & half_mask;
        buf >>= half_of_uint;

        buf += 
            a_high * b_low + 
            a_low * b_high + 
            (results[index] >> half_of_uint);
        unsigned new_high = buf & half_mask;
        buf >>= half_of_uint;

        results[index] = (new_high << half_of_uint) | new_low;

        buf += a_high * b_high;
        while (buf != 0) {
            buf += results[++index] & half_mask;
            new_low = buf & half_mask;
            buf >>= half_of_uint;

            buf += results[index] >> half_of_uint;
            new_high = buf & half_mask;
            buf >>= half_of_uint;
    ...
    for (int i = 0; i < first.get_digits_count(); ++i) {
        for (int j = 0; j < second.get_digits_count(); ++j) {
            mult_and_add_to_vec(
                first.get_digit(i),
                second.get_digit(j),
                i + j
        ...
        mult_and_add_to_vec(
            first.get_digit(i),
            static_cast<unsigned>(second._oldest_digit),
            i + second.get_digits_count()
        );
    }
    for (int j = 0; j < second.get_digits_count(); ++j) {
        mult_and_add_to_vec(
            static_cast<unsigned>(first._oldest_digit),
            second.get_digit(j),
            first.get_digits_count() + j
        );
    }
    ...
    delete first._other_digits;
    if (results.back() & (1 << ((sizeof(unsigned) << 3) - 1)) == 0) {
        first._oldest_digit = static_cast<int>(results[results.size() - 1]);

        first._other_digits = new unsigned[results.size()];
        *first._other_digits = results.size() - 1;
        std::memcpy(first._other_digits + 1, results.data(), sizeof(unsigned) * (results.size() - 1));
    }
    else {
        first._oldest_digit = 0;

        first._other_digits = new unsigned[results.size() + 1];
        *first._other_digits = results.size();
        std::memcpy(first._other_digits + 1, results.data(), sizeof(unsigned) * results.size());
    }
...
{
    constexpr int mask = (1 << ((sizeof(int) << 3) - 1));
    _oldest_digit ^= mask;
